...
import { strToBitumAtoms } from "helpers/strings";
import balanceConnector from "connectors/balance";

/**
 * FixedBitumInput is a simple numeric input that is assumed to **always** hold
 * a floating point number representing a BITUM amount (ie, an amount that
 * will be mutiplied by 1e8 to get to the actual atoms value).
 *
 * This is **not** affected by the global currencyDisplay state.
 *
 * Whenever possible, use the BitumInput component, as it is more flexible and
 * already manages the underlying input value in atoms.
 */
export const FixedBitumInput = ({ currencyDisplay, ...props }) =>
  <FloatInput {...{ ...props, unit: currencyDisplay, maxFracDigits: 8 }} />;

/**
 * BitumInput provides a way to receive bitum amount inputs. Instead of the usual
 * value/onChange pair, it uses amount/onChangeAmount to track values in bitum
 ...
 */
@autobind
class BitumInput extends React.Component {
  ...
  changeAmount(value) {
    const { unitDivisor } = this.props;
    const amount = !value ? 0 : strToBitumAtoms(value, unitDivisor);
  ...
  onChange(e) {
    ...
    if (value) {
      ...
      const { unitDivisor } = this.props;
      const amount = strToBitumAtoms(value, unitDivisor);
...
export default balanceConnector(BitumInput);
