export default () => (
  <div className="constition">
    <p>
      <em>Bitum</em>(/ˈdi:ˈkred/, /dɪˈkred/, dee-cred) is an open, progressive,
  ...
  for the public benefit, with a primary focus on cryptocurrency technology.
      {" "}<em>Bitum</em>, as a currency and as a project, is bound by the following set
  ...
  equitable and sustainable framework within which to achieve
    ...
    <ul>
      <li>
        ...
        {" "}<em>Bitum</em> shall be free and open source-software.</li>
      <li>
    ...
    everyone,
        {" "}<em>Bitum</em> shall comply with all relevant bodies of law in the
    ...
    number of coins in
        {" "}<em>Bitum</em> shall not change. The total maximum supply for
        {" "}<em>Bitum</em> is 20,999,999.99800912 coins, with a per-block subsidy that
    ...
    coins.</li>
      <li>
        ...
        {" "}<em>Bitum</em> being a store of value and attacks against it shall be
  ...
  development and voted on by the
      {" "}<em>Bitum Assembly</em> as they arise, as an effective proof-of-assembly
    (&ldquo;PoA&rdquo;), until such time PoA is integrated into the
    blockchain.</li>
      <li>The
        {" "}<em>Bitum Assembly</em> shall be composed of diverse Assembly members who
  ...
  functions of admission (
    ...
    Councils and Assembly members as necessary.</li>
      <li>Membership of the
        {" "}<em>Bitum Assembly</em> shall consist of Assembly members who have been
  ...
  initial development organization shall be
      {" "}<em>Bitum Holdings Group</em> LLC (&ldquo;DHG&rdquo;), a Nevis LLC that is
  ...
  RFP shall be removed. Contracted parties shall be paid exclusively in
      {" "}<em>bitum</em> (&ldquo;BITUM&rdquo;) at the current effective BITUM/USD rate at
