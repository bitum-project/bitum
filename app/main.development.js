...
logger.log("info", "Versions: Bitum: %s, Electron: %s, Chrome: %s",
...
ipcMain.on("clean-shutdown", async function(event){
  const stopped = await cleanShutdown(mainWindow, app, GetBitumdPID(), GetBitumwPID());
...
ipcMain.on("get-bitumd-logs", (event) => {
  event.returnValue = GetBitumdLogs();
});

ipcMain.on("get-bitumwallet-logs", (event) => {
  event.returnValue = GetBitumwalletLogs();
});

ipcMain.on("get-bitum-logs", (event) => {
...
ipcMain.on("get-last-log-line-bitumd", event => {
  event.returnValue = lastLogLine(GetBitumdLogs());
});

ipcMain.on("get-last-log-line-bitumwallet", event => {
  event.returnValue = lastLogLine(GetBitumwalletLogs());
});
...
app.on("ready", async () => {
  ...
  windowOpts.title = "Bitum - " + app.getVersion();
...
app.on("before-quit", (event) => {
  logger.log("info","Caught before-quit. Set bitumition as was closed");
