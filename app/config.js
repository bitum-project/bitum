...
export function initWalletCfg(testnet, walletPath) {
  ...
  if (!config.has("currency_display")) {
    config.set("currency_display","BITUM");
  }
...
export function getWalletCert(certPath) {
  var cert;
  certPath = getBitumdRpcCert(certPath);
  try {
...
export function getBitumdCert(bitumdCertPath) {
  if(bitumdCertPath)
    if(fs.existsSync(bitumdCertPath))
      return fs.readFileSync(bitumdCertPath);

  var certPath = getBitumdRpcCert();
...
export function newWalletConfigCreation(testnet, walletPath) {
  ...
  fs.writeFileSync(bitumwalletCfg(getWalletPath(testnet, walletPath)), ini.stringify(bitumwConf));
}
