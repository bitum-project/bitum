...
import { EXTERNALREQUEST_STAKEPOOL_LISTING, EXTERNALREQUEST_POLITEIA, EXTERNALREQUEST_BITUMDATA } from "main_dev/externalRequests";
...
export const currencies = () => [ { name: "BITUM" }, { name: "atoms" } ];
...
export const unitDivisor = compose(disp => disp === "BITUM" ? 100000000 : 1, currencyDisplay);
...
export const txURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.bitum.io/${network == "testnet" ? "explorer/" : ""}tx/${txHash}`
);

export const blockURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.bitum.io/${network == "testnet" ? "explorer/" : ""}block/${txHash}`
);
...
export const bitumdataURL = createSelector(
  [ isTestNet ],
  (isTestNet) => isTestNet ? BITUMDATA_URL_TESTNET : BITUMDATA_URL_MAINNET
);
...
export const bitumdataEnabled = compose(
  l => l.indexOf(EXTERNALREQUEST_BITUMDATA) > -1,
  allowedExternalRequests
);
