...
import { EXTERNALREQUEST_BITUMDATA } from "main_dev/externalRequests";
...
export const saveSettings = (settings) => (dispatch, getState) => {
  ...
  const oldBitumdataEnabled = oldAllowedExternalRequests.indexOf(EXTERNALREQUEST_BITUMDATA) > -1;
  const newBitumdataEnabled = settings.allowedExternalRequests.indexOf(EXTERNALREQUEST_BITUMDATA) > -1;
  if (newBitumdataEnabled === true && oldBitumdataEnabled === false) {
    dispatch(getTreasuryBalance());
  }
  if (newBitumdataEnabled === false && oldBitumdataEnabled === true) {
