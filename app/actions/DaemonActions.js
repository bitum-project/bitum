...
export const BITUM_VERSION = "BITUM_VERSION";
...
export const DELETE_BITUMD_ATTEMPT = "DELETE_BITUMD_ATTEMPT";
export const DELETE_BITUMD_FAILED = "DELETE_BITUMD_FAILED";
export const DELETE_BITUMD_SUCCESS = "DELETE_BITUMD_SUCCESS";
  ...
  axios.get(releaseApiURL, { timeout: 5000 })
    .then(function (response) {
      ...
      if (semverCompatible(currentVersion, detectedVersion)) {
        wallet.log("info", "Bitum version up to date.");
      } else {
        dispatch({ type: BITUM_VERSION, msg:  response.data[0].tag_name });
      }
    })
    .catch(function (error) {
      console.log("Unable to check latest bitum release version.", error);
    });
};
...
export const deleteDaemonData = () => (dispatch, getState) => {
  ...
  dispatch({ type: DELETE_BITUMD_ATTEMPT });
  wallet.deleteDaemonData(appData, isTestNet(getState()))
    .then(() => {
      dispatch({ type: DELETE_BITUMD_SUCCESS });
      dispatch(shutdownApp());
    })
    .catch((err) => dispatch({ err, type: DELETE_BITUMD_FAILED }));
};
...
export const getBitumdLogs = () => {
  wallet.getBitumdLogs()
    .then(logs => {
      return(logs);
    }).catch(
      err=>{
...
export const getBitumwalletLogs = () => {
  wallet.getBitumwalletLogs()
    .then(logs => {
      return(logs);
    }).catch(
      err=>{
