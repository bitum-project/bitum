...
import { model1_bitum_homescreen } from "helpers/trezor";
...
// Right now (2018-07-06) bitumwallet only supports a single account on watch only
...
// checkTrezorIsBitumwallet verifies whether the wallet currently running on
// bitumwallet (presumably a watch only wallet created from a trezor provided
...
// trezor wallet and then validating this address agains bitumwallet, ensuring
...
const checkTrezorIsBitumwallet = (session) => async (dispatch, getState) => {
  ...
  if (!addrValidResp.getIsMine()) throw "Trezor and bitumwallet not running from the same extended public key";
...
export const signTransactionAttemptTrezor = (rawUnsigTx, constructTxResponse) => async (dispatch, getState) => {
  ...
  try {
    ...
    const signedRaw = await deviceRun(dispatch, getState, device, async session => {
      await dispatch(checkTrezorIsBitumwallet(session));
...
// walletTxToBtcjsTx converts a tx decoded by the bitum wallet (ie,
...
export const walletTxToBtcjsTx = (tx, changeIndex, inputTxs) => async (dispatch, getState) => {
  ...
  for (const inp of tx.getInputsList()) {
    ...
    inputs.push({
      ...
      address_n: addressPath(addrIndex, addrBranch, WALLET_ACCOUNT,
        chainParams.HDCoinType),
      bitum_tree: inp.getTree()

      // FIXME: this needs to be supported on trezor.js.
      // bitumTree: inp.getTree(),
      // bitumScriptVersion: 0,
    });
  }
  ...
  for (const outp of tx.getOutputsList()) {
    ...
    outputs.push({
      ...
      address: addr,
      address_n: address_n,
      bitum_script_version: outp.getVersion(),
    });
  }
...
// walletTxToRefTx converts a tx decoded by the bitum wallet into a trezor
...
export const changeToBitumHomeScreen = () => async (dispatch, getState) => {
  const device = selectors.trezorDevice(getState());
  if (!device) {
    dispatch({ error: "Device not connected", type:  TRZ_TOGGLEPASSPHRASEPROTECTION_FAILED });
    return;
  ...
  try {
    await deviceRun(dispatch, getState, device, async session => {
      await session.changeHomescreen(model1_bitum_homescreen);
    });
