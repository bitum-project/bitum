...
import * as da from "../middleware/bitumdataapi";
import { EXTERNALREQUEST_BITUMDATA, EXTERNALREQUEST_POLITEIA } from "main_dev/externalRequests";
...
export const getStartupWalletInfo = () => (dispatch) => {
  ...
  const bitumdataEnabled = config.get("allowed_external_requests").indexOf(EXTERNALREQUEST_BITUMDATA) > -1;
...
export const getTransactions = () => async (dispatch, getState) => {
  ...
  // first, request unmined transactions. They always come first in bitum.
...
export const getTreasuryBalance = () => (dispatch, getState) => {
  ...
  const dURL = sel.bitumdataURL(getState());
  da.getTreasuryInfo(dURL, treasuryAddress)
    .then(treasuryInfo => {
      // Manually convert BITUM to atom amounts to avoid floating point multiplication errors (eg. 589926.57667882*1e8 => 58992657667881.99)
      const treasuryBalance = parseInt(treasuryInfo["data"]["bitum_unspent"].toString().replace(".",""));
